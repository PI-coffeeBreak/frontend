...
export function PageContent({
    ...
    const renderSections = () => {
        if (sections.length === 0) {
            return (
                <div className="text-center py-16 px-4 border-2 border-dashed border-base-300 rounded-xl">
            ...
            >
                <SortableContext items={sections.map((section) => section.id)} strategy={verticalListSortingStrategy}>
                    <div className="space-y-4 pb-8">
                        {renderSections()}
                        <button
                            className="flex items-center justify-center gap-2 w-full p-5 border-2 border-dashed border-base-300 rounded-xl
